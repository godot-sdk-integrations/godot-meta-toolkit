...
class State:
    ...
    def parse_class(self, class_root: ET.Element, filepath: str) -> None:
        class_name = realize_class_name(class_root.attrib["name"])
            ...
            for constant in constants:
                ...
                if enum is None:
                    if constant_name in class_def.constants:
                        ...
                        continue
                    if constant_def.text.strip() == '' and value == 'platform-dependent':
                        # Skip undocumented platform-dependent constants (probably just a local import).
                        continue
...
class TypeName:
    ...
    def from_element(cls, element: ET.Element) -> "TypeName":
        return cls(realize_class_name(element.attrib["type"]), element.get("enum"))
...
def realize_class_name(class_name: str) -> str:
    """ Converts class name from what we get in the XML, into what we want to show users. """

    if class_name.startswith('"'):
        # Clean up unnamed classes.
        class_name = ':'.join(class_name.rsplit('".', 1))
...
def sanitize_class_name(class_name: str) -> str:
    """ Sanitizes the realized class name so we can use it in labels and file names. """

    if class_name.startswith('res://'):
        class_name = class_name[6:]
        for x in [':', '/', '.']:
            class_name = class_name.replace(x, '_')
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    class_name = class_def.name
    class_name_cleaned = sanitize_class_name(class_name)

    if dry_run:
        f = open(os.devnull, "w", encoding="utf-8")
    else:
        f = open(os.path.join(output_dir, f'class_{class_name_cleaned}.rst'), "w", encoding="utf-8")
    ...
    f.write(".. Generated automatically from XML sources.\n")
    ...
    if len(class_def.properties) > 0:
        ...
        for property_def in class_def.properties.values():
            ...
            if default is not None and property_def.overrides:
                ...
                ref = f":ref:`{property_def.name}<class_{class_name_cleaned}_property_{property_def.name}>`"
    ...
    if len(class_def.theme_items) > 0:
        ...
        for theme_item_def in class_def.theme_items.values():
            ref = f":ref:`{theme_item_def.name}<class_{class_name_cleaned}_theme_{theme_item_def.data_name}_{theme_item_def.name}>`"
    ...
    if len(class_def.signals) > 0:
        ...
        for signal in class_def.signals.values():
            ...
            f.write(f".. _class_{class_name_cleaned}_signal_{signal.name}:\n\n")
    ...
    if len(class_def.enums) > 0:
        ...
        for e in class_def.enums.values():
            ...
            f.write(f".. _enum_{class_name_cleaned}_{e.name}:\n\n")
            ...
            for value in e.values.values():
                ...
                f.write(f".. _class_{class_name_cleaned}_constant_{value.name}:\n\n")
    ...
    if len(class_def.constants) > 0:
        ...
        for constant in class_def.constants.values():
            ...
            f.write(f".. _class_{class_name_cleaned}_constant_{constant.name}:\n\n")
            f.write(".. rst-class:: classref-constant\n\n")

            if constant.value != 'platform-dependent':
    ...
    if len(class_def.annotations) > 0:
        ...
        for method_list in class_def.annotations.values():  # type: ignore
            for i, m in enumerate(method_list):
                ...
                if i == 0:
                    f.write(f".. _class_{class_name_cleaned}_annotation_{m.name}:\n\n")
                ...
                if m.description is not None and m.description.strip() != "":
                    ...
                    f.write(
                        translate(
                            "There is currently no description for this annotation."
                        )
                        + "\n\n"
                    )
    ...
    if any(not p.overrides for p in class_def.properties.values()) > 0:
        ...
        for property_def in class_def.properties.values():
            ...
            f.write(f".. _class_{class_name_cleaned}_property_{property_def.name}:\n\n")
    ...
    if len(class_def.constructors) > 0:
        ...
        for method_list in class_def.constructors.values():
            for i, m in enumerate(method_list):
                ...
                if i == 0:
                    f.write(f".. _class_{class_name_cleaned}_constructor_{m.name}:\n\n")
    ...
    if len(class_def.methods) > 0:
        ...
        for method_list in class_def.methods.values():
            for i, m in enumerate(method_list):
                ...
                if i == 0:
                    f.write(f".. _class_{class_name_cleaned}_method_{m.name}:\n\n")
    ...
    if len(class_def.operators) > 0:
        ...
        for method_list in class_def.operators.values():
            for i, m in enumerate(method_list):
                ...
                operator_anchor = f".. _class_{class_name_cleaned}_operator_{sanitize_operator_name(m.name, state)}"
    ...
    if len(class_def.theme_items) > 0:
        ...
        for theme_item_def in class_def.theme_items.values():
            ...
            f.write(f".. _class_{class_name_cleaned}_theme_{theme_item_def.data_name}_{theme_item_def.name}:\n\n")
...
def make_type(klass: str, state: State) -> str:
    ...
    if link_type in state.classes:
        return f":ref:`{klass}<class_{sanitize_class_name(link_type)}>`"
    if link_type in CORE_TYPES:
        return klass
    ...
    if isinstance(definition, MethodDef) and ref_type != "":
        if ref_type == "operator":
            ...
            for parameter in definition.parameters:
                out += f"_{sanitize_class_name(parameter.type_name.type_name)}"
            out += f">` "
        else:
    ...
    for group_name in CLASS_GROUPS:
        if group_name in grouped_classes:
            ...
            for class_name in grouped_classes[group_name]:
                ...
                f.write(f"    {class_name} <class_{sanitize_class_name(class_name)}>\n")
...
def format_text_block(
    ...
    while True:
        ...
        realized_class_name = realize_class_name(tag_text)

        # Tag is a reference to a core type.
        if tag_text in CORE_TYPES:
            tag_text = f"**{tag_text}**"
            escape_pre = True
            escape_post = True

        # Tag is a reference to a class.
        elif realized_class_name in state.classes and not inside_code:
            tag_text = realized_class_name
                ...
                if link_target == "":
                    ...
                    if (
                        ...
                        if link_target.find(".") != -1:
                            link_target = realize_class_name(link_target)
                            if link_target.find(":") != -1:
                                file_name, inner_target = link_target.rsplit(":", 1)
                                ss = inner_target.rsplit(".", 1)
                                if len(ss) == 1:
                                    class_param = file_name
                                    method_param = ss[0]
                                else:
                                    class_param = file_name + ":" + ss[0]
                                    method_param = ss[1]
                            else:
                                ss = link_target.rsplit(".", 2)
                        ...
                        is_core_class = class_param in CORE_TYPES
                        ...
                        if is_core_class:
                            tag_text = f"**{class_param}.{method_param}**"
                        else:
                            repl_text = method_param
                            if class_param != state.current_class:
                                repl_text = f"{class_param}.{method_param}"
                            tag_text = f":ref:`{repl_text}<class_{sanitize_class_name(class_param)}{ref_type}_{method_param}>`"
                        escape_pre = True
                        escape_post = True
