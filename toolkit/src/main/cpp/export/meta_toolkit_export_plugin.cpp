...
namespace {
static const char *TOOLKIT_PREBUILT_DEBUG_TEMPLATE_PATH = "res://addons/godot_meta_toolkit/.build_template/quest_prebuilt_debug.apk";
static const char *TOOLKIT_PREBUILT_RELEASE_TEMPLATE_PATH = "res://addons/godot_meta_toolkit/.build_template/quest_prebuilt_release.apk";
...
Dictionary MetaToolkitExportPlugin::_get_export_options_overrides(
	...
	// Meta XR features overrides
	overrides["meta_xr_features/quest_1_support"] = false;
	overrides["meta_xr_features/quest_2_support"] = true;
	overrides["meta_xr_features/quest_3_support"] = true;
	overrides["meta_xr_features/quest_pro_support"] = true;
...
void MetaToolkitExportPlugin::_get_manifest_entries(Vector<godot::String> &r_permissions, Vector<MetaToolkitExportPlugin::FeatureInfo> &r_features) const {
...
PackedByteArray MetaToolkitExportPlugin::_update_android_prebuilt_manifest(const Ref<godot::EditorExportPlatform> &p_platform, const godot::PackedByteArray &p_manifest_data) const {
	...
	_get_manifest_entries(permissions, features);
	...
	while (ofs < (uint32_t)manifest_buffer.size()) {
		...
		switch (chunk) {
			...
			case CHUNK_XML_START_TAG: {
				...
				uint32_t attrcount = decode_uint32(&manifest_buffer[iofs + 20]);
				iofs += 28;

				for (uint32_t i = 0; i < attrcount; i++) {
					uint32_t attr_nspace = decode_uint32(&manifest_buffer[iofs]);
					uint32_t attr_name = decode_uint32(&manifest_buffer[iofs + 4]);
					uint32_t attr_value = decode_uint32(&manifest_buffer[iofs + 8]);
					uint32_t attr_resid = decode_uint32(&manifest_buffer[iofs + 16]);
					...
					// Update the meta-data tags.
					if (tname == "meta-data" && attrname == "value" && value == "meta_xr_hand_tracking_metadata_value") {
						...
						string_table.write[attr_value] = hand_tracking_frequency_label;
					}

					if (tname == "meta-data" && attrname == "value" && value == "meta_xr_ossplash_value") {
						bool instant_splash_screen = _get_bool_option("meta_xr_features/instant_splash_screen");
						string_table.write[attr_value] = _bool_to_string(instant_splash_screen);
					}

					if (tname == "meta-data" && attrname == "value" && value == "passthrough-contextual") {
						bool is_passthrough_contextual = (int)ProjectSettings::get_singleton()->get_setting_with_override("xr/openxr/environment_blend_mode") != XRInterface::XR_ENV_BLEND_MODE_OPAQUE;
						if (!is_passthrough_contextual) {
			...
			case CHUNK_XML_END_TAG: {
				...
				if (tname == "uses-feature") {
					if (features.size() > 0) {
						...
						for (int i = 0; i < features.size(); i++) {
							...
							int feature_version = features[i].version;
				...
				if (tname == "manifest") {
					...
					ERR_FAIL_COND_V_MSG(ns_android_string == -1, result, "Template does not have 'android' namespace.");
