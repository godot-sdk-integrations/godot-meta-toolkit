...
public:
	...
	bool should_display(ProjectType p_project_type) {
		bool matches_project_type = project_type == PROJECT_TYPE_ANY || project_type == p_project_type;
		return matches_project_type && !is_recommendation_applied();
	}
	...
	Recommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart) :
			title(p_title), description(p_description), button_text(p_button_text), alert_type(p_alert_type), project_type(p_project_type), requires_restart(p_requires_restart) {}
};
...
public:
	...
	SimpleProjectSettingRecommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart, String p_setting_path, Variant p_recommended_value) :
			Recommendation(p_title, p_description, p_button_text, p_alert_type, p_project_type, p_requires_restart), setting_path(p_setting_path), recommended_value(p_recommended_value) {}
};

class VendorsPluginRecommendation : public Recommendation {
public:
	bool is_recommendation_applied() override {
		return FileAccess::file_exists("res://addons/godotopenxrvendors/plugin.gdextension");
	}

	void on_button_pressed() override {
		MetaToolkitEditorPlugin *editor_plugin = MetaToolkitEditorPlugin::get_singleton();
		if (editor_plugin == nullptr) {
			return;
		}

		button->set_disabled(true);
		editor_plugin->open_asset_library("Godot OpenXR Vendors plugin v4");
	}

	VendorsPluginRecommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart) :
			Recommendation(p_title, p_description, p_button_text, p_alert_type, p_project_type, p_requires_restart) {}
};

class MetaToolkitEnabledRecommendation : public Recommendation {
public:
	bool is_recommendation_applied() override {
	...
	MetaToolkitEnabledRecommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart) :
			Recommendation(p_title, p_description, p_button_text, p_alert_type, p_project_type, p_requires_restart) {}
};
...
public:
	...
	SdkPathRecommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart, String p_sdk_setting_name) :
			Recommendation(p_title, p_description, p_button_text, p_alert_type, p_project_type, p_requires_restart), sdk_setting_name(p_sdk_setting_name) {}
};
...
public:
	...
	void on_button_pressed() override {
		...
		start_xr->store_string(String(start_xr_script_content));
		start_xr->close();
	...
	StartupSceneRecommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart, String p_xr_startup_scene_content, String p_start_xr_script_content) :
			Recommendation(p_title, p_description, p_button_text, p_alert_type, p_project_type, p_requires_restart), xr_startup_scene_content(p_xr_startup_scene_content), start_xr_script_content(p_start_xr_script_content) {}
};
...
public:
	...
	BootSplashRecommendation(String p_title, String p_description, String p_button_text, AlertType p_alert_type, ProjectType p_project_type, bool p_requires_restart) :
			Recommendation(p_title, p_description, p_button_text, p_alert_type, p_project_type, p_requires_restart) {}
};
} //namespace godot_meta_toolkit_internal
...
void MetaProjectSetupDialog::_notification(uint32_t p_what) {
	switch (p_what) {
		case NOTIFICATION_POSTINITIALIZE: {
			...
			// All project type recommendations
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("OpenXR", "OpenXR must be enabled", "Apply", ALERT_TYPE_ERROR, PROJECT_TYPE_ANY, true, "xr/openxr/enabled", true)));
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("XR Shaders", "XR shaders must be compiled", "Apply", ALERT_TYPE_ERROR, PROJECT_TYPE_ANY, true, "xr/shaders/enabled", true)));

			recommendations.push_back(memnew(VendorsPluginRecommendation("Vendors Plugin", "Please install the Godot OpenXR Vendors Plugin", "Open", ALERT_TYPE_ERROR, PROJECT_TYPE_ANY, false)));

			recommendations.push_back(memnew(MetaToolkitEnabledRecommendation("Meta Toolkit", "No Android export preset with Meta Toolkit enabled was found", "Open", ALERT_TYPE_ERROR, PROJECT_TYPE_ANY, false)));

			recommendations.push_back(memnew(SdkPathRecommendation("Java SDK", "Please set a valid Java SDK path in Editor Settings", "Info", ALERT_TYPE_ERROR, PROJECT_TYPE_ANY, false, "export/android/java_sdk_path")));
			recommendations.push_back(memnew(SdkPathRecommendation("Android SDK", "Please set a valid Android SDK path in Editor Settings", "Info", ALERT_TYPE_ERROR, PROJECT_TYPE_ANY, false, "export/android/android_sdk_path")));

			recommendations.push_back(memnew(BootSplashRecommendation("Boot Splash", "No valid boot splash image was found", "Open", ALERT_TYPE_WARNING, PROJECT_TYPE_ANY, false)));

			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("MSAA", "Recommended to set MSAA to 2x", "Apply", ALERT_TYPE_WARNING, PROJECT_TYPE_ANY, false, "rendering/anti_aliasing/quality/msaa_3d", 1)));
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("VRS Mode", "Recommended to set VRS mode to VR", "Apply", ALERT_TYPE_WARNING, PROJECT_TYPE_ANY, false, "rendering/vrs/mode", 2)));
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("Foveation Level", "Recommended to set foveation level to high", "Apply", ALERT_TYPE_WARNING, PROJECT_TYPE_ANY, false, "xr/openxr/foveation_level", 3)));
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("Dynamic Foveation", "Recommended to enable dynamic foveation", "Apply", ALERT_TYPE_WARNING, PROJECT_TYPE_ANY, false, "xr/openxr/foveation_dynamic", true)));

			// Virtual reality recommendations
			recommendations.push_back(memnew(StartupSceneRecommendation("Main Scene", "No main scene is set, add VR startup scene?", "Add", ALERT_TYPE_WARNING, PROJECT_TYPE_VR, false, vr_startup_tscn, start_vr_gd)));

			// Mixed reality recommendations
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("Passthrough", "Meta passthrough extension should be enabled for mixed reality", "Apply", ALERT_TYPE_WARNING, PROJECT_TYPE_MR, false, "xr/openxr/extensions/meta/passthrough", true)));
			recommendations.push_back(memnew(SimpleProjectSettingRecommendation("Environment Blend Mode", "Environment blend mode should be non-opaque for mixed reality", "Apply", ALERT_TYPE_WARNING, PROJECT_TYPE_MR, false, "xr/openxr/environment_blend_mode", 2)));

			recommendations.push_back(memnew(StartupSceneRecommendation("Main Scene", "No main scene is set, add MR startup scene?", "Add", ALERT_TYPE_WARNING, PROJECT_TYPE_MR, false, mr_startup_tscn, start_mr_gd)));
			...
			project_type_hbox->set_h_size_flags(Control::SIZE_EXPAND | Control::SIZE_SHRINK_BEGIN);
			top_vbox->add_child(project_type_hbox);
			...
			project_type_selector->connect("item_selected", callable_mp(this, &MetaProjectSetupDialog::_on_project_type_selected));
...
void MetaProjectSetupDialog::add_window_entry(Recommendation *p_recommendation) {
	...
	button->connect("pressed", callable_mp(this, &MetaProjectSetupDialog::_on_recommendation_button_pressed).bind(reinterpret_cast<uint64_t>(p_recommendation)));

	HSeparator *separator = memnew(HSeparator);
	rec_vbox->add_child(separator);
}

void MetaProjectSetupDialog::filter_recommendations() {
	ProjectType project_type = ProjectType(project_type_selector->get_selected_id());
	for (Recommendation *recommendation : recommendations) {
		if (recommendation->should_display(project_type)) {
...
void MetaProjectSetupDialog::_on_recommendation_button_pressed(uint64_t p_recommendation) {
...
void MetaProjectSetupDialog::_on_project_type_selected(int p_item_index) {
...
MetaProjectSetupDialog::MetaProjectSetupDialog() {
	...
	connect("focus_entered", callable_mp(this, &MetaProjectSetupDialog::filter_recommendations));
}
